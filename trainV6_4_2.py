...
if datasetName==1: 
    ...
    fileName='HM36_[1, 5, 6, 7, 8, 9]_Inte_50.npy'
    fileNametest='HM36_[11]_Inte_50.npy'
...
net=nn.Sequential(
    nn.Conv2d(in_channels=3,out_channels=64,dilation=2,kernel_size=5,padding=4),
    yy.myNN.ResNetBackbone(block_type, layers,in_channel=64),
...
picture=yy.drawSomeLines(2,xlim=[0,epochSize],ylim=[0,600],interval=int(num_batches/40),mode=2)
stop=False
for epoch in range(epochSize):
    if epoch == 0:
        yy.myNN.changeLr(optimizer,1e-4)
    elif epoch == 4:
        yy.myNN.changeLr(optimizer,0.1e-4)
    elif epoch == 9:
        yy.myNN.changeLr(optimizer,0.01e-4)
    net.train()
